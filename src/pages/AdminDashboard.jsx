...
export default function AdminDashboard() {
  ...
  async function handleAddProduct() {
    ...
    await supabase.from('productos').insert([{ name, price, stock }]);
    fetchProductos();
  }
                  ...
                  <tr key={p.id} className="border-b border-[#E6DCC8]/5">
                    {editingId === p.id ? (
                      <>
                        ...
                        <td className="p-4"><button onClick={() => handleUpdate(p.id)} className="text-[#25D366] font-bold">Guardar</button></td>
                      </>
                    ) : (
                      <>
